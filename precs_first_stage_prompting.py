...
pair_score_path = "dataset/genai/precs/without.json"
...
for id in tqdm( q_ids):
    ...
    for i, p_id in enumerate(precs_rank_list[id][:10]):
        ...
        try:
            out= llm.invoke(ss.only_precs_chat_template.format_messages(query_txt=query_text, precs_txt=prec_text))
...
torch.save(llm_only_precs_scores,"scores/gen_ai/precs_without_secs.pt")
