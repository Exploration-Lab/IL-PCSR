...
class EUGATConv(nn.Module):
    ...
    def __init__(
        ...
        if residual:
            ...
            self.res_fc_edge = nn.Linear(
                edge_feats, num_heads * out_edge_feats, bias=False
            )            
        else:
...
##################################################################################
        self.fc_ni = nn.Linear(
            self._in_src_feats, out_edge_feats * num_heads, bias=False
        )
        ...
        self.fc_fij = nn.Linear(
            edge_feats, out_edge_feats * num_heads, bias=False
        )
